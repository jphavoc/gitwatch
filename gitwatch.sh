...
if [ -z "$GW_INW_BIN" ]; then INW="fswatch -1 -r -x --exclude .DS_Store --exclude .git --event Created --event Removed --event MovedTo --event MovedFrom --event Renamed --event Updated"; else INW="$GW_INW_BIN"; fi
...
if [[ -d $1 ]]; then # if the target is a directory
    ...
    INCOMMAND="$INW $TARGETDIR" # construct inotifywait-commandline
    ...
    INCOMMAND="$INW $IN" # construct inotifywait-commandline
